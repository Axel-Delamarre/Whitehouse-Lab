...
- **All output should be in Markdown** suitable for inclusion in notebooks or online documentation.
- Each step must include a rationale/explanation _before_ listing the commands to ensure conceptual clarity.
- Steps should be clearly separated, logically grouped, and include original URLs, filenames, and command lines as in the source.

---

## Example of Workflow Step Formatting
...
**Reasoning:**  
[Explanation of why this step is performed.]
...
- Ensure all required tools are functional before starting the workflow.
- Command outputs and file naming conventions must be carefully maintained to avoid downstream errors.
- Parameter values (e.g., `--clip_R1 40`, linker sequence patterns) may require adjustment for different experiments.
- Refer to linked documentation for tool-specific troubleshooting.

---

**Reminder:**  
Objective – Compile the PCP data processing workflow into a clear, full-length Markdown protocol. Each step must include an explanation before the action(s), preserving all constants, tool links, commands, and tips from the description within a smoothly organized Markdown document. Output should be a single, readable `.md` file.
