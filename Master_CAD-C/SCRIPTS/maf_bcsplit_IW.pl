...
print("query,target,dir,qS,qE,qML,qL,qPct,tS,tE,tML,tL,tPct,barcodePosition,score");
...
while(<>){
  ...
  if($F[0] eq "a"){
    ...
    if($tSeq){
      ...
      my $matchLine =
        sprintf("%s,%s,%s,%d,%d,%d,%d,%0.2f,%d,%d,%d,%d,%0.2f,%d",
                $qName, $tName, $qStrand,
                $qStart, $qEnd, $qMatchLen, $qLen, ($qMatchLen / $qLen) * 100,
                $tStart, $tEnd, $tMatchLen, $tLen, ($tMatchLen / $tLen) * 100, $qStartb);
