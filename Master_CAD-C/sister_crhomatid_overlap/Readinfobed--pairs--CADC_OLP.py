...
path= ''
read_info= 'olp.read_info.txt'


#----> OUTPUT files

plt_size_distribution = read_info + '_size-ditribution_test.pdf'

plt_fragment_per_CCF_total = path + read_info + '_fragments_per_CCF_total_test.pdf'
plt_fragment_per_CCF_nosingle = path + read_info + '_fragments_per_CCF_nosingle_test.pdf'

tojuice = path + read_info +'_test.tojuice'
juicer = path + read_info +'_test.juicer'
...
df_reads = pd.read_table(read_info, names=['chr', 'read_start', 'read_stop', 'length', 'read_midpoint', 'bc'])
...
df_hic = df_reads[['bc','chr','read_midpoint','length']]

#df_hic = df_hic.dropna(subset=['midpoint'])

# Get it back to the fromat BC  chr-pos-length
df_hic['concatenated'] = df_hic.apply(lambda row: '-'.join([str(row['chr']), str(row['read_midpoint']), str(row['length'])]), axis=1)
...
with open(juicer, 'w') as juicer:
    ...
    for i in range(len(arr)):
        ...
        for j in comb:
            ...
            juicer.write(str(str1) + ' ' + chr1 + ' ' + str(pos1) + ' ' + str(frag1) + ' ' + str(str2) + ' ' + chr2 + ' ' + str(pos2) + ' ' + str(frag2) + ' ' + barcode +'\n')
